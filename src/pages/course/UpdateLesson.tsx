...
import { adminUpdateLessons } from '../../reduxKit/actions/admin/adminLessonAction';
...
useEffect(() => {
   ...
   const fetchDocument = async () => {
      try {
        const response = await commonRequest(
          "GET",
          `/admin/getLessonById/${id}`,
          config,
          {}
        );
    
        if(response.data.success){
            console.log("Lesson with id : ",response.data.data);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        console.log("before update lesson  lesson : ",);
        
    const response=  await dispatch(adminUpdateLessons({id:id,data:formDataToSend})).unwrap();
  ...
  // Reset form on success
      ...
      alert('Error Updated lesson. Please try again.');
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Update  Lesson</h1>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4 pt-6 border-t border-gray-200">
              ...
              >
                {lessonLoading ? 'Updating Lesson...' : 'Update Lesson'}
              </button>
              <button
                type="button"
                onClick={resetForm}
